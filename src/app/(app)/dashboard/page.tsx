...
import { Loader2, BookOpen, AlertTriangle, Sparkles, ChevronRight } from "lucide-react";
...
export default function DashboardPage() {
  ...
  if (error && (!profile || !profile.name)) { 
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-theme(spacing.20))] text-center mt-0">
  ...
  return (
    <div className="mt-0">
      <div className="mb-6 text-center pt-0 mt-0">
      ...
      {subjects.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {subjects.map((subject, index) => (
            <Card key={index} className="flex flex-col overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 bg-card/80 backdrop-blur-sm border-border/50 hover:border-primary/50">
              <CardHeader className="p-4 pb-2">
                <div className="relative h-40 w-full mb-3 rounded-md overflow-hidden group">
                  <Image 
              ...
              <CardContent className="flex-grow p-4 pt-2">
                {subject.studyMaterials && subject.studyMaterials.length > 0 && (
                  <div>
                    <h4 className="font-semibold mb-1 text-sm text-primary">Key Topics:</h4>
