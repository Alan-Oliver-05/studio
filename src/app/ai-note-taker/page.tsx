...
const inputTypeOptions: InputTypeOption[] = [
  ...
  { value: "recording", label: "Audio", icon: Mic2, title:"AI Audio Note Taker & Q&A", description: "Unpack lectures and meetings. Upload an audio file, and the AI will transcribe and summarize key discussions and insights from the actual audio content.", placeholder: "Upload your audio file (e.g., .mp3, .wav).", storageTopic: "Audio Content Summarization & Q&A" },
...
export default function SummarizerPage() {
  ...
  const handleAudioFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        toast({ title: "Audio File Ready", description: `Ready to process "${file.name}".`});
      };
      reader.onerror = () => {
  ...
  const handleSummarizeAudio = async () => {
    if (!uploadedAudioFile || !uploadedAudioDataUri || !profile) {
      toast({ title: "Missing Input", description: "Please upload an audio file and wait for it to be ready.", variant: "destructive" });
      return;
    }
    ...
    try {
      const aiInput: AIGuidedStudySessionInput = {
        ...
        question: `Summarize the audio file named: ${uploadedAudioFile.name}`,
        originalFileName: uploadedAudioFile.name,
        audioDataUri: uploadedAudioDataUri,
      };
      const result = await aiGuidedStudySession(aiInput);
  ...
  const handleAskAudioQuestion = async () => {
    if (!currentAudioQuestion.trim() || !uploadedAudioFile || !uploadedAudioDataUri || !profile || !currentMediaConversationId) {
        ...
        return (
            ...
            >
                ...
                {!uploadedAudioFile && <p className="text-xs text-muted-foreground">{currentInputTypeConfig.placeholder}</p>}
                 <p className="text-xs text-muted-foreground mt-2">Max file size: 25MB. Supported: MP3, WAV, M4A etc.</p>
            </div>
        );
      case "powerpoint":
         return (
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mt-6 text-center">
            ...
            disabled={isLoading || isProcessingPdf || isProcessingAudio || isProcessingSlides || isProcessingVideo || isParsingPdf ||
              ...
              (activeInputType === "recording" && (!uploadedAudioFile || !uploadedAudioDataUri)) ||
        ...
        <Card className="mt-8 shadow-lg max-w-3xl mx-auto bg-card/90 backdrop-blur-sm">
          ...
          <CardContent className="space-y-6">
            ...
            {(audioProcessingOutput || audioQuestionHistory.length > 0) && ( 
             ...
             <div className="text-xs text-muted-foreground pt-4 border-t mt-2"><Info className="inline h-3.5 w-3.5 mr-1.5 align-middle"/>AI responses for audio are based on the full audio content you upload.</div>
          </CardContent>
        </Card>
      )}
